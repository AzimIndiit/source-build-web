...
export const PickupHoursSelector: React.FC<PickupHoursSelectorProps> = React.memo(({ value, onChange }) => {
  ...
  const parseFormattedString = useCallback((str: string): Record<string, DayHours> | null => {
    try {
  ...
  const formatPickupHours = useCallback((hrs: Record<string, DayHours>, hols: string[] = []): string => {
  ...
  const applyToAll = useCallback((sourceDay: string) => {
